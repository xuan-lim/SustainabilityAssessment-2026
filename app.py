...
"hrdd17": {
    ...
    def run(self):
            # This hidden script forces the main content area to scroll to the top
            # We use the step number as the key so it re-triggers every time you change pages
            components.html(
                f"""
                <script>
                ...
                """,
                height=0,
                key=f"scroll_top_{st.session_state.step}"
            )
